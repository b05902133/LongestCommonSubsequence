...
sub initDatabase
{
  ...
  our $mDatabase  = $self->{database};
  our $mStringA   = $self->{stringA};
  our $mStringB   = $self->{stringB};
...
sub collectResults
{
  ...
  if( @_ )
  {
    ...
    our $data     = ${ ${ $self->{database} }[$i] }[$j];
    our @mResults = @{ $self->{results} };
...
{
  my ( $self, $i, $j ) = @_;
  my $dataDec = dataSource( $i, $j, 'dec_ab' );
  ...
  if( $charA eq $charB )
  {
    $data->setLcsLength( $data->length + 1 );
...
sub removeOneSide
{
  ...
  my $dataDec = dataSource( $i, $j, $source );
